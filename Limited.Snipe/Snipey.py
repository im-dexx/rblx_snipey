...
if not os.path.exists(f"bin/nostartup.dex"):
    if not os.path.exists(
        f"C:/Users/{os.getlogin}/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup/Startup/LimitedSnipe.pyw"
    ):
        print(banner)
        print(
            """[RBLX.Snipe - Notification]
        ...
        if onstartup == "y":
            try:
                ...
                sniperstartup = open(
                    f"C:/Users/{os.getlogin}/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/Startup/LimitedSnipe.pyw",
                    "w",
                )
                sniperstartup.write(
                    """
...
def sniper():
    ...
    assetprice = int(input("Desired Asset Price: "))
    asset = requests.get(
        robloxapi + f"marketplace/productinfo?assetId={assetid}"
    ).json()
    print(
...
Asset Availability (As of now): {str(asset["IsForSale"])}
    ...
    while True:
        if asset["IsForSale"] == True and asset["PriceInRobux"] <= assetprice or asset["PriceInRobux"] == assetprice:
            if saleandunder == 0:
                print(f"[+]: Asset for sale and is under {str(assetprice)}")
                saleandunder = 1
                webbrowser.open("https://www.roblox.com/catalog/{}/".format(assetid))
            else:
                pass
        elif asset["IsForSale"] == True and asset["PriceInRobux"] >= assetprice or asset["PriceInRobux"] != assetprice:
            if salebutover == 0:
                print(f"[+]: Asset for sale but is over {str(assetprice)}")
                salebutover = 1
            else:
                pass
        else:
